...
def _allocate_compact_arrays(
  ...
  alloc_dense = compact and not is_sparse(mjm)

  d.cM = wp.empty((nw, nvp, nvp), dtype=float) if alloc_dense else wp.empty((0, 0, 0), dtype=float)
  d.cqLD = wp.empty((0, 0, 0), dtype=float)
  d.crhs = wp.empty((0, 0, 0), dtype=float)
  d.cx = wp.empty((0, 0, 0), dtype=float)
  d.cJ = wp.empty((nw, njp, nvp), dtype=float) if alloc_dense else wp.empty((0, 0, 0), dtype=float)
