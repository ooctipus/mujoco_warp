...
class FusedWorldTest(absltest.TestCase):
  ...
  def test_forward_worlds_matches_stock(self):
    ...
    for name in self._FORWARD_WORLDS_FLOAT_FIELDS:
      if name in ("cacc", "cfrc_int", "qfrc_bias"):
        # RNE outputs: the fused ancestor-walk sums differ from the stock level sums in summation
        # order, so cancellation residuals are compared relative to the field scale
        self._assert_close_scaled(
          name, getattr(d_sub, name).numpy()[selected], getattr(d_ref, name).numpy()[selected], rel=1e-4
        )
        continue
      atol = 1e-5 if name == "crb" else _ATOL
